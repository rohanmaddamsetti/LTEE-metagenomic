...
mutator_parallelism_axis.set_ylabel(r'Fraction mutations $\geq m$',y=1.1)
...
vartype_pfix_axis.set_ylabel(r'$\mathrm{Pr}[\mathrm{fixed}|\mathrm{detected}]$')
...
nonmutator_pfix_axis.set_ylabel(r'$\mathrm{Pr}[\mathrm{fixed}|\mathrm{detected}]$')
...
mutator_pfix_axis.set_ylabel(r'$\mathrm{Pr}[\mathrm{fixed}|\mathrm{detected}]$')
...
for metapopulation in metapopulations:
    ...
    for include_svs in [True, False]:
        ...
        binned_num = numpy.array([0 for i in range(0,len(bins)-1)])
        binned_response = numpy.array([0 for i in range(0,len(bins)-1)])
        ...
        binned_pfix_upper = numpy.array([beta.ppf(0.84, binned_response[i]+syn_pfix, binned_num[i]-binned_response[i]+(1-syn_pfix), loc=0, scale=1) for i in range(0,len(binned_num))])
        
        # New version: plot average
        binned_pfix_mid = numpy.array([binned_response[i]*1.0/binned_num[i] for i in range(0,len(binned_num))])
        ...
        binned_pfix_lower = numpy.array([beta.ppf(0.16, binned_response[i]+syn_pfix, binned_num[i]-binned_response[i]+(1-syn_pfix), loc=0, scale=1) for i in range(0,len(binned_num))])
        ...
        parallelism_axes[include_svs][metapopulation].step(late_predictors, (len(late_predictors)-numpy.arange(0,len(late_predictors)))*1.0/len(late_predictors), where='post',color=colors[metapopulation],alpha=0.25,label=r'$>\mathrm{Median}(t)$')
    
        parallelism_axes[include_svs][metapopulation].step(early_predictors, (len(early_predictors)-numpy.arange(0,len(early_predictors)))*1.0/len(early_predictors), where='post',color=colors[metapopulation],alpha=0.75,label=r'$\leq \mathrm{Median}(t)$')
...
nonmutator_axis.set_xticklabels(["" for i in range(0,9)])

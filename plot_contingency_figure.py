...
pooled_time_axis.set_ylabel(r'Fraction mutations $\geq t$',fontsize=6)
...
LRT_axis.set_ylabel(r'LRT statistic, $\\Delta \ell$',fontsize=6)
...
twohit_axis.set_ylabel(r'Fraction of genes $\geq \Delta t$',fontsize=6)
twohit_axis.set_xlabel(r'Time difference, $\Delta t$', fontsize=6)
...
def calculate_time_distribution(gene_names, times):
    ...
    for gene_name in list(time_distribution.keys()):
...
def calculate_population_distribution(gene_names, pops):
    ...
    for gene_name in list(pop_distribution.keys()):
...
for bootstrap_idx in range(1,num_bootstraps+1):
...
parallel_ks_pvalues = numpy.array([stats_utils.calculate_empirical_pvalue(parallel_kss[i],bootstrapped_kss[:,i]) for i in range(0,len(parallel_genes))])
...
upper_null_LRTs = []
for i in range(0,len(tstars)):

    LRTs = numpy.array(bootstrapped_LRTs[:,i],copy=True)
    LRTs.sort()
    
    upper_null_LRTs.append( LRTs[int(len(LRTs)*0.95)] )       
...
grey = True
for i in range(0,len(parallel_genes)):
...
for i in range(0,len(desired_genes)):
...
lower_early_dms = []
for i in range(0,bootstrapped_early_dms.shape[1]):
...
upper_late_dms = []
for i in range(0, bootstrapped_late_dms.shape[1]):
    
    null_dms = numpy.array(bootstrapped_late_dms[:,i],copy=True)
    null_dms.sort()
    upper_late_dms.append( null_dms[int(len(null_dms)*0.95)] )
...
missed_opportunity_axis.plot(tstars, early_dms, '-', color=early_color,label=r'$\leq t^*$')
...
for tmin, tmax, matrix_axis in zip([-1000, -1000, tstar],[65000, tstar, 65000], [all_matrix_axis, early_matrix_axis, late_matrix_axis]):
    ...
    for hlower, h, hupper in [(1.5,2,2.5),(2.5,3,3.5),(3.5,4,4.5),(4.5,5,5.5),(5.5,6,100.5)]:
        ...
        for idx in desired_idxs:
            ...
            bootstrapped_counts = numpy.array([(bootstrapped_num_pops[:,idx]==k).sum() for k in range(1,6+1)])
    ...
    for row_idx in range(0,len(missed_opportunity_matrix)):
        for col_idx in range(0,len(missed_opportunity_matrix[row_idx])):
...
#cbar.set_label(r'Excess probability, $\Delta P$ (%)',rotation=270,labelpad=10) 
cax.text(0.5,-0.07,r'Excess\nprobability\n $\Delta P$ (%)',horizontalalignment='center',verticalalignment='top')
####
#
# Figure labels
#

...
[i.set_linewidth(0.75) for i in mutator_mut_axis.spines.values()]
...
late_s_axis.set_ylabel(r'Late gain, $\Delta X(t)/ \Delta M(t)$ (%)',labelpad=2)
...
xticks = [10000*i for i in range(4,7)]
xticklabels = ['%dk' % (10*i) for i in range(4,7)]
...
late_fitness_axis.set_ylabel(r'Fitness gain\n$\Delta X$ (%)',labelpad=2)
...
correlation_pvalue_axis.set_xlabel(r'Between line stddev, $\sigma_M$', labelpad=2)
correlation_pvalue_axis.set_ylabel(r'Fraction bootstraps $\geq \sigma_M$', labelpad=2)
...
transit_time_axis.set_xlabel(r'Transit time, $\\Delta t$')
transit_time_axis.set_ylabel(r'Fraction mutations $\\geq \\Delta t$')
...
for population in list(trajectories.keys()):
...
for i in range(0,len(measured_populations)):
...
for t in avg_dMts:
    dM_map[t].sort()
    lower_dMs.append( dM_map[t][int(len(dM_map[t])*0.025)])
    upper_dMs.append( dM_map[t][int(len(dM_map[t])*0.975)])
...
for bootstrap_idx in range(2,2+num_bootstraps):
...
velocity_axis.plot(avg_dMts,avg_dMs,'w-',linewidth=1,zorder=14,path_effects=[pe.Stroke(linewidth=2, foreground='k'), pe.Normal()],label=r'$\\Delta t = 5k$')

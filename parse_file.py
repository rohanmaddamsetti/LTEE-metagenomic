...
def parse_operon_list(filename="additional_data/door_operon_list.txt"): #, gene_data=None, repeat_data=None, position_gene_map=None):
    ...
    for line in file:
        ...
        start_position = int(items[3])
        end_position = int(items[4])
    ...
    for operon_id in list(operons.keys()):
    ...
    for gene_name in list(gene_operon_map.keys()):
...
def create_gene_size_map(effective_gene_lengths=None):
    ...
    if effective_gene_lengths is None:
        reference_sequence = parse_reference_genome()
        gene_data = parse_gene_list()
        repeat_data = parse_repeat_list()
        mask_data = parse_mask_list()
...
def parse_barrick_data():
    ...
    for t,p in zip(barrick_mutation_times,barrick_mutation_counts):
        mutations[('m1',t)] = [[str(i) for i in range(0,p)]]
    return mutations
...
def parse_wielgoss_data(directory='wielgoss_genomes'):
    ...
    for filename in filenames:  
        ...
        if True:
            generation = int(filename[4:6])*1000
...
def parse_tenaillon_data(directory='additional_data/tenaillon_genomes/LTEE-clone-curated'):
    ...
    for filename in filenames:  
        ...
        generation = int(items[1][:-3])
        clone_str = items[2]
        
        
        if population_str.startswith('Anc'):
            if population_str[3]=='+':
                populations = ['p%d' % i for i in range(1,7)]
            else:
                populations = ['m%d' % i for i in range(1,7)]    
        else:
            if population_str[3]=='-':
...
def get_tenaillon_mutation_trajectories():
    ...
    for population,t in list(tenaillon_data.keys()):
        ...
        for i in range(0,len(tenaillon_data[(population,t)])):
    ...
    for population in list(mutation_trajectories.keys()):
...
def get_tenaillon_mutation_trajectory(masked_populations=[]):
    ...
    avg_ts = final_mutation_trajectories[list(final_mutation_trajectories.keys())[0]][0]
...
def get_mutation_trajectory(allowed_lines = all_lines): 
    ...
    for population,t in list(wielgoss_data.keys()):
    ...
    for population,t in list(barrick_data.keys()):
        if population in allowed_lines:
            if not t in muts:
                muts[t] = []
...
def get_40k_gains():
    ...
    ss = []
    ds2s = []
    for line,i in zip(complete_nonmutator_lines,list(range(1,1+len(complete_nonmutator_lines)))):
...
def get_all_40k_gains():
    ...
    for line,i in zip(complete_lines,list(range(1,1+len(complete_lines)))):
        x40 = trajectories[line][1][0]
        dx40 = trajectories[line][2][0]
        x50 = trajectories[line][1][1]
        dx = (x50-x40)
...
def parse_tenaillon2012_mutations(filename="additional_data/TenaillonEtal12_mutations.csv"):
    ...
    for line in file:
        ...
        if tenaillon_var_type == "deletion":
            allele = "indel;-%d" % int(allele.split("_")[0])

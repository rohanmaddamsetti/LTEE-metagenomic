...
pooled_G_scores = numpy.array(list(gene_G_scores.values()),copy=True)
...
pooled_multiplicities = numpy.array([gene_parallelism_statistics[gene_name]['multiplicity'] for gene_name in list(gene_parallelism_statistics.keys())])
...
pooled_hits = numpy.array([gene_parallelism_statistics[gene_name]['observed'] for gene_name in list(gene_parallelism_statistics.keys())])
pooled_hits.sort()    
...
pooled_pvalues = []
for gene_name in list(gene_logpvalues.keys()):

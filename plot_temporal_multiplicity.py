...
multiplicity_axis.set_ylabel(r'Fraction mutations $\geq m$')
...
for i in range(0,len(tstars)):
    ...
    upper_early_dGs.append( dGs[int(len(dGs)*0.975)] )
    
    dGs = numpy.array(bootstrapped_late_dGs[:,i],copy=True)
    dGs.sort()
    
    lower_late_dGs.append( dGs[int(len(dGs)*0.025)] )
...
g_axis.plot(tstars, early_dGs,'-',color=early_color, label=r'$\leq t^*$')
...
multiplicity_axis.step(early_survival_ms, early_survivals*1.0/early_survivals[0],color=early_color,label=(r'$\leq t^* = %g\mathrm{k}$' % (tstar/1000.0)))
multiplicity_axis.step(late_survival_ms, late_survivals*1.0/late_survivals[0],color=late_color,label=(r'$> t^* = %g\mathrm{k}$' % (tstar/1000.0)))

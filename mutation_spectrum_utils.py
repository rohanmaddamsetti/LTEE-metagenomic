...
def calculate_parallelism_statistics(convergence_matrix, allowed_populations=parse_file.complete_nonmutator_lines,Lmin=0):
    ...
    Lavg = Ltot*1.0/len(list(convergence_matrix.keys()))
    
    for gene_name in list(gene_statistics.keys()):
...
def calculate_G_scores(gene_statistics, allowed_genes=None):
    ...
    gene_G_scores = {gene_name: gene_statistics[gene_name]['observed']*gene_g_scores[gene_name] for gene_name in list(gene_g_scores.keys())}
...
def calculate_total_parallelism(gene_statistics, allowed_genes=None, num_bootstraps=10000):
    ...
    if allowed_genes is None:
        allowed_genes = list(gene_statistics.keys())
...
class NullMultiplicitySurvivalFunction(object):
    ...
    def from_parallelism_statistics(cls, gene_parallelism_statistics):
        ...
        Ls = []
        ntot = 0
        for gene_name in list(gene_parallelism_statistics.keys()):
...
def calculate_clade_grouping_loglikelihood(hit_table,ws):
    ...
    clade_associations = []
    for association_idx in range(0,total_associations):
        group_1 = [1,0]
        group_2 = [0,1]
        for bit in range(0,p-1):
...
def excess_zeros_test(hit_table,ws,num_bootstraps=1000,alpha=0.05):
    ...
    percentile_idx = int(len(bootstrapped_excess_zeros)*alpha/2)

...
gene_name_position_map = {gene_names[i]: (start_positions[i], end_positions[i]) for i in range(0,len(gene_names))}
...
xticks = [5000*i for i in range(0,13)]
xticklabels = ['%dk' % (5*i) for i in range(0,13)]
xticklabels[0] = '0'
    

freq_axis = None
    
for population_idx in range(0,len(populations)):        
    ...
    for mutation_idx in range(0,len(mutations)):
        ...
        if color_condition(population_idx, location, gene_name, var_type, interpolation_function, haplotype_interpolation_function, extra_data):
            ...
            if PLOT_GENE_LABEL: ## I added this line to label trajectories.
                xcoord = t
                if xcoord > 57000: ## try this hack to prevent labels from going off the page.
                    xcoord = 57000
                ycoord = float(interpolation_function(t)) + 0.1
                xycoord = (xcoord, ycoord)
                ##print xycoord, gene_name
                freq_axis.annotate(text=gene_name,xy=xycoord)
